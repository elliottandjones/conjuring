...
export default function CreatureItem({ creature, chatOpen, onOpenChatPanel }) {
  ...
  return (
    <div className={`ma1 pa1 creature-item ${isExpanded ? 'inset creature-ex' : 'outset'}`}>
      ...
      >
        <div
          className="item-body dib pa2 ccard hotem"
          ref={heightRef}
          aria-expanded={isExpanded}
          aria-hidden={!isExpanded}
        >
          ...
          <p>
            <b className="mr2">Armor Class</b> {creature.armor_class}
          </p>
          <p>
            <b className="mr2">Hit Points</b> {creature.hit_points} ({creature.hit_dice})
          </p>
          <p>
            <b className="mr2">Speed</b> {creature.speed}
          </p>
          ...
          <div className="tc ability-scores">
            ...
            <div>
              {creature.strength} {calculateModifier(creature.strength)}
            </div>
            <div>
              {creature.dexterity} {calculateModifier(creature.dexterity)}
            </div>
            <div>
              {creature.constitution} {calculateModifier(creature.constitution)}
            </div>
            <div>
              {creature.intelligence} {calculateModifier(creature.intelligence)}
            </div>
            <div>
              {creature.wisdom} {calculateModifier(creature.wisdom)}
            </div>
            <div>
              {creature.charisma} {calculateModifier(creature.charisma)}
            </div>
          </div>
          <div className="from-center">
            <i>.</i>
          ...
          {whatSkills(creature) !== '' && (
            <p>
              <b>Skills</b> {whatSkills(creature)}
            </p>
          )}
          {creature.damage_vulnerabilities && (
            <p>
              <b>Damage Vulnerabilities</b> {creature.damage_vulnerabilities}
            </p>
          )}
          {creature.damage_resistances && (
            <p>
              <b>Damage Resistances</b> {creature.damage_resistances}
            </p>
          )}
          {creature.damage_immunities && (
            <p>
              <b>Damage Immunities</b> {creature.damage_immunities}
            </p>
          )}
          {creature.condition_immunities && (
            <p>
              <b>Condition Immunities</b> {creature.condition_immunities}
            </p>
          )}
          <p>
            <b>Senses</b> {creature.senses}
          </p>
          {creature.languages && (
            <p>
              <b>Languages</b> {creature.languages}
            </p>
          )}
          <p>
